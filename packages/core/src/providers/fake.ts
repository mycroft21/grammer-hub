...
function fakeStudio(input: ProviderInput): unknown | null {
  ...
  if ("success_criteria" in props && "hard_rules" in props) {
    ...
    return {
      ...
      context: null,
      hard_rules: en ? ["Do not guess behavior of files you have not read", "Do not mix refactoring proposals into findings"] : ["읽지 않은 파일의 동작을 추측하지 않는다", "리팩터링 제안을 조사 결과에 섞지 않는다"],
      process: null,
      ...
      failure_guards: en ? ["Do not infer a function's role from its name alone; check the call sites"] : ["이름만 보고 역할을 단정하지 않는다(실제 호출을 확인한다)"],
