...
5. **대상이 사람이 아니라 코딩 에이전트다** — ✅ 반영(스튜디오 v0.5.0). 이 문서의 출처는 대부분 "사람이 채팅 모델에게 주는 프롬프트" 기준이다. 실제 대상(Claude Code·Codex)에 맞춰 두 벤더의 공식 지침과 Copilot·Cursor·Devin 등의 작업 작성 가이드, 질문 시점 연구를 따로 조사해 [docs/11 코딩 에이전트 프롬프트 벤치마크](11-coding-agent-prompt-benchmark.md)에 정리했다. 거기서 나온 변화: 완료 조건에 실행 가능한 확인, 목적별 범위 유지 문장, 보고 형식(결과부터·검증은 실행 결과로), 시작점은 '저장소: 대상', 질문은 "저장소를 읽어 알 수 있는 것은 묻지 않는다" 기준으로 최대 2개, 저장소 지식은 프롬프트가 아니라 작업 공간 프로필에.
