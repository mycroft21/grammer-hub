...
| 코드 점검 8개(checks.ts) | 위 원칙을 정규식·개수로 검사 | 원칙에서 파생. 임계값(기준 3개 이상, 규칙 5개 이하, 구성 5자 이상)은 우리 판단 |

---

## 5. 이 조사로 드러난 고칠 점

1. **부정 표현 규칙**: 방어 지침·절대 규칙을 "X 하지 않는다"로만 쓰지 말고 "X 대신 Y"로 쓰도록 메타 프롬프트에 한 줄 추가(A-BP format 권고). → 다음 씨앗 손질 때 함께.
2. **예시 기본값**: 공식 권고(3~5개)와 우리 기본값("필요할 때만")이 다르다. 비용을 보며 `detailed` 모드는 예시 1개 이상을 기본으로 올리는 것을 검토.
