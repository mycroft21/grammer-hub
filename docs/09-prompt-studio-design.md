...
| 질문 정책 (v0.5) | **필요 정보 장부(needs ledger)**로 고정. 모델은 항목마다 `filled / ask / assume / agent_can_find` 중 하나만 고르고, 질문·가정·"코드에서 확인할 것"은 코드가 장부에서 만든다(`needs.ts`). 기준: 저장소를 읽어 알 수 있는 것은 절대 묻지 않는다(→ 프롬프트의 확인 항목). 사람만 아는 것(어느 저장소·업무 규칙·범위 결정·첨부에만 있는 정보) 중 결과물을 바꾸는 것만, **최대 2개**. 항목 = 공통 6개(where·deliverable·done·scope·external·policy) + 세부 유형의 mustKnow. 첫 실사용에서 세 티켓의 질문 행동이 제각각(묻고 또 가정, 첨부 있는데 안 묻기)이던 것을 보고 바꿈. 근거: [docs/11](11-coding-agent-prompt-benchmark.md) §4. |
| 작업 공간 프로필 (v0.5) | 루트 `studio.workspace.json`(예시 `studio.workspace.example.json`, gitignore). 저장소(이름·설명·스택·별칭·진입점·검증 명령·주의), Jira 프로젝트 뜻, 팀 규칙(≤5), 용어, 기본값. **코드가** 제목·라벨·컴포넌트(다음은 본문)에서 별칭을 찾아 대상 저장소를 확정하고, 확정되면 where를 묻지 않는다. 못 정하면 프로필 저장소가 질문 선택지. 분류·의도 정리·생성·재생성 프롬프트에 같은 "작업 공간" 블록이 들어간다(용어는 텍스트에 나온 것만). Claude Code의 CLAUDE.md / Codex의 AGENTS.md가 저장소 안에서 하는 일을 저장소 밖(티켓→프롬프트)에서 한다. **편집(v0.6)**: 설정 화면의 폼(저장소 카드·키-값 표·규칙 목록, zod 경로 → 칸 오류) + JSON 탭 + 가져오기/내보내기(팀 프로필 배포). 검토 화면의 "프로필에 추가"는 `ProfileOp`(add_repo·add_alias·add_verify) 하나씩을 `PATCH /api/settings/workspace`로 보내 병합한다 — 프로필을 미리 완성하지 않아도 쓰면서 쌓인다. 별칭 후보는 코드가 고른다(`suggestAliases`: 제목의 대괄호 태그 → 라벨 → 컴포넌트, 이미 아는 것 제외). 저장소를 하나만 골랐을 때만, 코드가 확정한 저장소에는 별칭을 제안하지 않는다. |
| 실행 환경 추가 (v0.5) | `codex` 추가. 슬롯·점검은 `claude_code`와 같고(`isAgentRuntime`) 렌더만 다르다: Codex CLI는 system 프롬프트가 없어 `<system>` 태그 없이 한 덩어리. |
| 에이전트 프롬프트 형태 (v0.5) | 메타 프롬프트 0.5.0 + 렌더. 에이전트 런타임 제목은 Codex 4부(Goal→Context→Constraints→Done when)와 Claude Code 권고에 맞춤: **범위와 제약 / 보고 형식 / 주의 / 정보가 부족할 때** ‖ **목표 / 맥락 / 시작점 / 완료 조건 / 진행 / 끝내기 전에 검증**. 코드가 넣는 고정 문장: 목적별 범위 유지 한 줄(`agent-defaults.ts`: 조사·설계·검토는 "코드를 수정하지 않는다", 구현은 "요청된 변경만, 무관한 문제는 후속 항목, 기존 테스트 삭제·변조 금지"), 보고 한 줄("결과부터, 검증은 명령과 출력으로, 못 한 것은 따로"), 시작점 한 줄("어떤 파일을 읽고 어떤 명령을 돌릴지는 스스로 정한다"). 결과물 분량은 목적별 표로 통일(조사 20~40줄 / 설계 1,000~1,500자 / 구현 10줄+검증 출력 / 검토 항목당 2~3줄) — 프롬프트 길이 모드(short·standard)와 분리. `self_check`는 에이전트에서 "끝내기 전에 실행할 검증(명령·증거)"으로 의미가 바뀐다('…했는가?' 되묻기 금지 — 과잉 검증 방지). 점검 3개 추가: `starting_points`가 이름 하나뿐이면 실패, `verification_runnable`(완료 조건·검증에 실행 가능한 확인이 있는가), `scope_consistent`(조사·계획·검토 분류인데 목표·진행이 구현이면, 또는 구현 분류인데 '코드를 수정하지 않고 설계안만'이면 실패 — 코드가 넣는 범위 문장·보고 분량과 모순되므로). `verification_runnable`은 목적을 안다: 구현은 테스트·빌드·재현, 조사·설계·검토는 '읽어서 인용·확인'도 인정. 티켓 흐름은 장부의 결과물 형태(문서 vs 코드 변경)와 분류가 어긋나면 검토 화면에 "분류를 X로 바꾸는 것이 맞아 보입니다" + 한 번에 바꾸기 버튼(`suggested_purpose`). |
| 사람 이름 (v0.5) | Jira 응답의 보고자·담당자·작성자·댓글 작성자·멘션 표시명을 본문·댓글에서 역할명으로 바꾼다(`redactPeople`). PII 마스킹은 패턴이 있는 것(전화·이메일)만 잡기 때문. |
...
| E2E | `apps/web/e2e/smoke.mjs` 뒤쪽: 질문 → 답변 생성 → 14 블록 → 보관 → 보관함 변수 채워 복사 → 영어 지시문(에이전트 제목) → DEMO-1(프로필로 저장소 확정, 첨부 질문 1개, 장부) → DEMO-2(제목뿐인 티켓: 별칭으로 저장소 확정, 업무 판단 질문 1개, 가정으로 진행) |

### 11-5. 근거

슬롯·원칙·배치가 어디서 왔는지, 무엇이 공식 문서 근거이고 무엇이 우리 판단인지는 [docs/10-prompt-engineering-references.md](10-prompt-engineering-references.md)에 항목별로 적었다. 거기서 드러난 고칠 점(부정 표현 규칙, 예시 기본값, 영어 지시문 A/B)은 §5에 있다. **코딩 에이전트(Claude Code·Codex)를 대상으로 한 v0.5 규칙·질문 정책·프로필의 근거는 [docs/11](11-coding-agent-prompt-benchmark.md).**
