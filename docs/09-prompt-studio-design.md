...
| 에이전트 프롬프트 형태 (v0.5) | 메타 프롬프트 0.5.0 + 렌더. 에이전트 런타임 제목은 Codex 4부(Goal→Context→Constraints→Done when)와 Claude Code 권고에 맞춤: **범위와 제약 / 보고 형식 / 주의 / 정보가 부족할 때** ‖ **목표 / 맥락 / 시작점 / 완료 조건 / 진행 / 끝내기 전에 검증**. 코드가 넣는 고정 문장: 목적별 범위 유지 한 줄(`agent-defaults.ts`: 조사·설계·검토는 "코드를 수정하지 않는다", 구현은 "요청된 변경만, 무관한 문제는 후속 항목, 기존 테스트 삭제·변조 금지"), 보고 한 줄("결과부터, 검증은 명령과 출력으로, 못 한 것은 따로"), 시작점 한 줄("어떤 파일을 읽고 어떤 명령을 돌릴지는 스스로 정한다"). 결과물 분량은 목적별 표로 통일(조사 20~40줄 / 설계 1,000~1,500자 / 구현 10줄+검증 출력 / 검토 항목당 2~3줄) — 프롬프트 길이 모드(short·standard)와 분리. `self_check`는 에이전트에서 "끝내기 전에 실행할 검증(명령·증거)"으로 의미가 바뀐다('…했는가?' 되묻기 금지 — 과잉 검증 방지). 점검 3개 추가: `starting_points`가 이름 하나뿐이면 실패, `verification_runnable`(완료 조건·검증에 실행 가능한 확인이 있는가), `scope_consistent`(조사·계획·검토 분류인데 목표·진행이 구현이면 실패 — 코드가 넣는 "코드를 수정하지 않는다"와 모순되므로. 설계+구현 티켓은 구현으로 분류하고 진행 앞에 설계 단계를 둔다). |
