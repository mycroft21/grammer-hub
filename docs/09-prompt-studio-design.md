...
| 작업 공간 프로필 (v0.5) | 루트 `studio.workspace.json`(예시 `studio.workspace.example.json`, gitignore). 저장소(이름·설명·스택·별칭·진입점·검증 명령·주의), Jira 프로젝트 뜻, 팀 규칙(≤5), 용어, 기본값. **코드가** 제목·라벨·컴포넌트(다음은 본문)에서 별칭을 찾아 대상 저장소를 확정하고, 확정되면 where를 묻지 않는다. 못 정하면 프로필 저장소가 질문 선택지. 분류·의도 정리·생성·재생성 프롬프트에 같은 "작업 공간" 블록이 들어간다(용어는 텍스트에 나온 것만). Claude Code의 CLAUDE.md / Codex의 AGENTS.md가 저장소 안에서 하는 일을 저장소 밖(티켓→프롬프트)에서 한다. |
