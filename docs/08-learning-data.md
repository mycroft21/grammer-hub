...
실제 교정을 시작하기 전에 그래프와 수집 화면이 어떻게 보이는지 확인하려면:

```bash
pnpm seed:demo              # 8주치 데모 데이터 주입 (실행 20~40회, 샘플 3개)
pnpm seed:demo -- --weeks 4 # 기간 지정
pnpm seed:clear             # 데모 데이터만 삭제
```

- 모든 행의 id가 `demo-`로 시작하므로 `seed:clear`가 **실제 데이터는 건드리지 않고** 데모만 정확히 지운다.
- 주차가 갈수록 수락률이 조금씩 오르도록 만들어 추이 선이 보인다. 카테고리 분포도 실제로 나올 법한 교정(사물 존대, `-ㄹ게요`, 의존명사 띄어쓰기, 과잉 완곡 표현)으로 구성했다.
- 비용·토큰은 Sonnet 5 단가로 계산해 넣으므로 건당 ₩10~25 수준이 나온다. 실측이 아니라 시뮬레이션이다.
- **실제 사용 기록과 섞이면 학습 신호가 오염된다.** 진짜 교정을 시작하기 전에 `pnpm seed:clear`를 실행할 것. 데이터셋 내보내기에도 데모가 섞여 나간다.
