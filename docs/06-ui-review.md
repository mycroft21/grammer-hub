...
## 7. A안 적용 결과 (2026-09-17)

![A안 적용 후 에디터](assets/ux-review/after-a-editor.png)

적용한 것:
- 전역: 기본 테두리색 `neutral-200`, Pretendard Variable(동적 서브셋), 14px/1.6, `word-break: keep-all`, 등폭 숫자.
- 툴바: 강도 라벨을 "맞춤법만 / 다듬기 / 다시 쓰기"로, 세그먼트 컨트롤 도입, 글자수는 보조 정보로.
- 결과 패널: **"최종본 복사"를 헤더 우상단으로**, 완료 시 초록 강조, `⌘⇧C` 단축키. 개발 지표는 하단 상태줄 `2.4초 · ₩31 · 캐시 48%`로 축소(툴팁에 토큰 상세).
- 본문 하이라이트: 대기=카테고리 점선 밑줄만, 수락=교정문 + 연한 초록, 무시=표시 없음. 색은 4계열(정확성 빨강 / 높임·문체 보라 / 명확성 파랑 / 어조 주황).
- 변경 카드: 한 줄 요약(점 · 카테고리 · 원문→교정 · ✓ ✕) + 포커스 시 펼침. 오류는 채운 점, 제안은 빈 점. 카드 목록은 자체 스크롤(최대 52vh)이라 페이지가 길어지지 않음.
- 톤 대안: 세그먼트 3개 + 본문 하나. "이걸로 교체"는 헤더에.
- provider 배지는 하단 상태줄의 점으로. 로컬이면 "내 Mac에서 처리".
- 빈 상태: "예시 문장 넣기" + 단축키 안내. 고칠 곳이 없으면 "그대로 보내도 됩니다".
- 모바일: 교정 시작 시 원문을 자동으로 접고(펼치기 버튼), 상단 메뉴 단어 끊김 해결.
...
![B안 모바일](assets/ux-review/after-b-mobile.png)
...
- **E2E 9개 통과.** 하이드레이션 전에 입력하면 React 상태에 반영되지 않는 경합이 있어(AntD 번들로 하이드레이션이 길어짐) 교정 버튼이 활성화될 때까지 입력을 재시도하도록 보강. 셀렉터는 `data-testid=run/copy/result-text`, `data-done`, `aria-label`, `data-rewrite`, `tr.ant-table-row`로 고정.
